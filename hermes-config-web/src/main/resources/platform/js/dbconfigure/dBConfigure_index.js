...
$(function(){
	...
	vue = new Vue({
		...
		data: function(){
			return {
				...
				form: {dbName:null,username:null,dbKey:null,url:null,password:null},
		        formRules: {
					dbName:[
		...
		methods:{
			...
			edit: function(){//编辑
				...
				PlatformUI.ajax({
					url: contextPath + "/dBConfigure/" + ids[0],
			...
			exp: function(){//导出
				PlatformUI.exportGrid("list", "from etl_db_configure");
			},
			resetForm: function(){
				this.dialogFormVisible = false;
				this.$refs['form'].resetFields();
				this.form = {dbName:null,username:null,dbKey:null,url:null,password:null};
			},
			onSubmit: function(){//弹出表单的提交
        		...
        		this.$refs['form'].validate(function(valid){
        			if (valid) {
        				...
        				var actionUrl = contextPath + "/dBConfigure";
        				if(context.formEdit){
        					actionUrl = contextPath + "/dBConfigure/" + data.id;
				            data['_method'] = "put";
        				}
        				PlatformUI.ajax({
			            	url: actionUrl,
	...
	grid = $("#list").jqGrid({
        ...
        mtype: "GET",
        multiselect: true,
        colNames: ['ID','配置名称','用户名','配置KEY','url','创建时间'],
        colModel: [
			{ name: 'id', index:'id',hidden: true},
			{ name: 'dbName', index:'dbName', align:'center', sortable: true},
			{ name: 'username', index:'username', align:'center', sortable: true},
			{ name: 'dbKey', index:'dbKey', align:'center', sortable: true},
